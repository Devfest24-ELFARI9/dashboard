...
const Alert: React.FC<Notification & {onClick: () => void}> = ({
  ...
  return (
    <div
      className={`flex w-full border-l-6 ${borderColor} ${bgColor} px-7 py-8 shadow-md dark:bg-[#1B1B24] dark:bg-opacity-30 md:p-9`}
    >
      <div
        className={`mr-5 flex h-9 w-9 items-center justify-center rounded-lg ${iconColor}`}
      >
        {icon}
      </div>
      <div className="w-full">
        <h5 className={`mb-3 text-lg font-semibold ${textColor}`}>{status}</h5>
        <p className={`leading-relaxed ${messageColor}`}>
          <b>
            <span className="text-xl font-extrabold">{machine_name}</span> :{" "}
            {alert_message}
          </b>
        </p>
      </div>
      {!fixed && (
        <div className="ml-auto flex items-center">
          <button
            ...
            className="flex h-8 w-16 transform items-center justify-center rounded-md bg-green-500 text-white transition-transform hover:scale-105"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-5 w-5"
